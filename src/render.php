...
if ( ! class_exists( 'Telex_Hours_Block_Renderer' ) ) {
	...
	class Telex_Hours_Block_Renderer {
		...
		/**
		 * Renders multiple time slots as HTML spans, comma-separated.
		 *
		 * @since 0.1.0
		 *
		...
		public function render_slots_html( array $slots, bool $friendly_twelves, string $day_short_key = '' ): string {
			...
			foreach ( $slots as $slot ) {
				...
				$schema_attr = '';
				if ( ! empty( $day_short_key ) ) {
					$schema_open  = gmdate( 'G:i', strtotime( $open ) );
					$schema_close = gmdate( 'G:i', strtotime( $close ) );
					$schema_value = $day_short_key . ' ' . $schema_open . '-' . $schema_close;
					$schema_attr  = ' itemprop="openingHours" content="' . esc_attr( $schema_value ) . '"';
				}

				$html  = '<span class="telex-hours-block__slot"' . $schema_attr . '>';
				$html .= '<span class="telex-hours-block__time telex-hours-block__time--open">' . esc_html( $display_open ) . '</span>';
				$html .= '<span class="telex-hours-block__separator"> &ndash; </span>';
				$html .= '<span class="telex-hours-block__time telex-hours-block__time--close">' . esc_html( $display_close ) . '</span>';
				$html .= '</span>';

				$parts[] = $html;
			}

			return implode( '<span class="telex-hours-block__slot-separator">, </span>', $parts );
		}
		...
		public function render_week( ?array $season, array $holidays, string $today_key, bool $friendly_twelves, DateTime $today, bool $show_reason, bool $hide_weekends = false ): string {
			...
			$html = '<ol class="telex-hours-block__list">';
			...
			foreach ( $day_keys as $dk ) {
				...
				$classes = array( 'telex-hours-block__list-item' );
				if ( $is_today ) {
					$classes[] = 'telex-hours-block__list-item--today';
				}
				$classes[] = $is_closed
					? 'telex-hours-block__list-item--closed'
					: 'telex-hours-block__list-item--open';

				$html .= '<li class="' . esc_attr( implode( ' ', $classes ) ) . '" data-day="' . esc_attr( $dk ) . '">';
				$html .= '<span class="telex-hours-block__day">' . esc_html( $day_labels[ $dk ] ) . '</span>';
				...
				if ( $is_closed ) {
					...
					$html .= '<span class="telex-hours-block__hours">' . $closed_label . '</span>';
				} else {
					$html .= '<span class="telex-hours-block__hours">';
